...
/**
 * API session List 현재, 과거 날짜 기준
 * 파라미터 없을 시 현재 세션을 가져옴
 * 
 */
app.get("/sessionlist", async (req, res) => {
  ...
  const options = {
    ...
    // url: 'https://api.zoom.us/v2/vidiosdk/session',

    headers: {
      // authorization: `Bearer ${JWT_TOKEN}`, // Do not publish or share your token publicly.
      authorization: `Bearer ${authorization}`, // Do not publish or share your token publicly.
